...
passport.deserializeUser(function(id, passDone){
  ...
  pg.connect(connection, function(err, client, pgDone){
    ...
    client.query('SELECT * FROM registered_users WHERE id = $1', [id], function(err, result){
      pgDone();
...
passport.use('local', new LocalStrategy(
  {
    ...
    usernameField: 'username'
      ...
      pg.connect(connection, function(err, client, pgDone){
        ...
        client.query('SELECT * FROM registered_users WHERE username = $1', [username], function(err, result) {
          pgDone();
